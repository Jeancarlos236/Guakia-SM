...
# ?check if the logic of being logged to se a profile is the better approach
def post_list_profile(request, id):
    ...
    is_requesting_user = request.user.id == user.id

    posts = Post.objects.filter(created_by_id=id)

    if not request.user in user.friends.all() and not is_requesting_user:
    ...
    if request.user in user.friends.all() or is_requesting_user:
